"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LoggerType = void 0;
const graphql_1 = require("@nestjs/graphql");
const log_type_1 = require("./log.type");
const user_type_1 = require("../../user/model/user.type");
let LoggerType = class LoggerType {
};
__decorate([
    graphql_1.Field((type) => graphql_1.ID),
    __metadata("design:type", Number)
], LoggerType.prototype, "id", void 0);
__decorate([
    graphql_1.Field(type => [log_type_1.LogType]),
    __metadata("design:type", Array)
], LoggerType.prototype, "logs", void 0);
__decorate([
    graphql_1.Field(),
    __metadata("design:type", Date)
], LoggerType.prototype, "createdAt", void 0);
__decorate([
    graphql_1.Field(),
    __metadata("design:type", Date)
], LoggerType.prototype, "updatedAt", void 0);
__decorate([
    graphql_1.Field(type => user_type_1.UserType),
    __metadata("design:type", user_type_1.UserType)
], LoggerType.prototype, "user", void 0);
LoggerType = __decorate([
    graphql_1.ObjectType("Logger")
], LoggerType);
exports.LoggerType = LoggerType;
//# sourceMappingURL=logger.type.js.map