...
let AuthProvider = class AuthProvider {
    ...
    async authorizeByToken(token) {
        ...
        let id;
        try {
            const payload = jsonwebtoken_1.verify(token, config.jwt.secret);
            id = payload["id"];
        }
        catch (err) {
            this.logger.warn(`Client failed to authorize: invalid token`);
            throw new common_1.UnauthorizedException("You are unauthorized");
        }
    ...
    async checkPassword(password, realPassword) {
